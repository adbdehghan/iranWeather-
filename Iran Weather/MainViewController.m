...
-(void)SendSMS
{
    if ([MFMessageComposeViewController canSendText]) {
        ...
        NSString *message =@"021"; //[NSString stringWithFormat:@"%@",cityId];
...
- (void)setBlurredOverlayImage:(UIImage*)_image
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^ {
        ...
        dispatch_async(dispatch_get_main_queue(), ^ {
            ...
            UIImageView *jadid = [[UIImageView alloc]initWithImage:blurred];
...
- (void)UpdateWeatherViewWithData:(WeatherData *)wd weatherView:(WeatherView *)weatherView
{
    ...
    //weatherView.windDirectionAngle = wd.windDirection;
...
-(NSString*)ConvertTemperature:(NSString*)temperature
{
    if (![temperature isEqual:@""]) {
        if ([StateManager temperatureScale]==CelsiusScale) {
            return [temperature stringByAppendingString:@" º C"];
        }
        else
        {
            double temp = [temperature doubleValue];
            return [[NSString stringWithFormat:@"%.0f",((9/5)*temp)+32] stringByAppendingString:@" º F"];
        }
    }
    return @"";
}
